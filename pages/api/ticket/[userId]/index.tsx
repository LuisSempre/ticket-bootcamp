...
export default function TicketPage({ user }: TicketPageProps) {
  ...
  const shareMessage = encodeURIComponent(
    `Fala, dev! Estou participando da Maratona Explorer, um evento online e gratuito de programação da Rocketseat, onde vamos construir juntos uma aplicação completa de 06 a 12 de junho e acelerar na carreira.\n\nJunte-se a mim pelo link: ${shareUrl}`
  );
  ...
  return (
    <main className="lg:h-screen max-w-[480px] lg:max-w-[1240px] p-12 mx-auto flex items-center gap-24 flex-col lg:flex-row">
      ...
      <div className="max-w-[520px] text-center lg:text-left">
        ...
        <strong className="inline-flex gap-4 mt-12 text-xl text-neutral-200 lg:flex">
          <img src="/icons/check.svg" />
          <span>
            Ticket criado <span className="text-green-500">com sucesso</span>
          </span>
        </strong>

        <strong className="block mt-4 text-4xl text-neutral-200">
          Agora mostre ao mundo
        </strong>

        <footer className="grid grid-cols-4 gap-6 mt-6">
          <a
        ...
        <button
          ...
          className="flex items-center justify-center w-full gap-2 px-6 py-4 mt-8 font-bold border rounded group text-brand-500 border-brand-500 hover:bg-brand-500 hover:text-black disabled:hover:bg-transparent disabled:hover:text-brand-500"
        >
